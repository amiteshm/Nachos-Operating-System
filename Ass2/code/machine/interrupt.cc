...
//------------------------------------------------------------------------------
for (ptr = kernel->scheduler->readyList->first; ptr != NULL; ptr = ptr->next) {
    	if((unsigned int)ptr->item->getstatus()==2){
    		//ptr->item->Wait++;
    		ptr->item->LastWait++;
    		if(ptr->item->LastWait &&(ptr->item->LastWait %100==0)&&(ptr->item->priority > 5) && (ptr->item->priority < 8)){
    		ptr->item->priority++;
    		}
    		if(ptr->item->LastWait &&(ptr->item->LastWait %300==0)&&(ptr->item->priority >= 0) && (ptr->item->priority <=5)){
    		ptr->item->priority++;
    		}
    	}
}
//------------------------------------------------------------------------------------------------------
   /*temp=kernel->scheduler->readyList->Next();
	//cout<<"\n*****checking prepemption needed or not :"<<temp->getName()<<" :" << temp->priority<<"******\n";
   if(temp->priority>kernel->currentThread->priority){
	cout<<"\n\n\n *************Preempting ***************\n\n";
...
// check any pending interrupts are now ready to fire
    ...
    if (yieldOnReturn) {	// if the timer device handler asked for a context switch, ok to do it now
		...
		cout <<"\n"<<kernel->currentThread->getName()<<" is about to yield\n";
		kernel->currentThread->Yield();
		status = oldStatus;
    }
}
...
void
Interrupt::PrintMessage()
{
	ASSERT(inHandler == TRUE);  
    cout << "This should be printed every 100 ticks\n";
}

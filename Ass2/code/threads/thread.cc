...
Thread::Sleep (bool finishing)
{
    ...
    status = BLOCKED;								//put in blocked list
...
void
Thread::SleepTest(int delay){
	SleepTimer *mySleep;
	mySleep = new SleepTimer(this,delay);
	IntStatus oldLevel = kernel->interrupt->SetLevel(IntOff);
	Sleep(FALSE);
	(void) kernel->interrupt->SetLevel(oldLevel);
	return;
...
void
Thread::Exp(){
	cout<<"888888888888888888888888888888888888888888888888888888888888888888888888888888888888888";
	return;
}
...
/*
void
Thread::WakeupAfter(int sleepTime){
	int a=sleepTime;
	kernel->interrupt->Schedule(this, 50, TimerInt);
	IntStatus oldLevel = kernel->interrupt->SetLevel(IntOff);
	Sleep(FALSE);
	(void) kernel->interrupt->SetLevel(oldLevel);
}
...
char*
Thread::NameCreator(){
	int numt = kernel->Serial++;
	altName[0] = 'P';altName[1] = 'r';altName[2] = 'o';altName[3] = 'c';
	altName[4] = (char)(numt+65);
	char *altname = altName;
	return altname;
}
	

SleepTimer::SleepTimer(Thread *t,int delay)
{
	toCall=t;
	myInterrupt(delay);
}
...
void 
SleepTimer::myInterrupt(int delay)
{
...
int
Thread::Attach(int size){
		if(size<=128){
			if(size>kernel->SharedMemSize){
				kernel->SharedMemSize=size;
			}
		}
		else{
			return 0;
		}
		space->pageTable[No_pr-1].physicalPage=NumPhysPages-1;
		space->RestoreState();
		SharedMemStatus=1;
		return ((No_pr-1)*PageSize);
}

void 
Thread::Detach(){
	space->pageTable[No_pr-1].physicalPage=(kernel->currentThread->space->Epage)+No_pr-1;
	space->RestoreState();
	SharedMemStatus=0;
}

...
template <class T>
void
List<T>::Applyreturn(void (*func)(T)) const
{ 
    ListElement<T> *ptr;
...
List<T>::Next()
{
	...
	final=first;
	for (ptr = first; ptr != NULL; ptr = ptr->next) {
			if(high <= ptr->item->priority)/*&&(ptr->item->getstatus()==READY))*/{
			final=ptr;
				//k=1;
				high=final->item->priority;
			}
	}
/*/	final->item->Lflag=0;
...
T thing=Next();
Remove(thing);
cout<<"\n****Next to Run :"<<thing->getName()<<"    Priority"<<thing->priority<<"****\n";
return thing;
}
